...
def remove_background(path, _always_matting, width_resize = 320, aspect_ratio = 1.285, visualize = False):
    ...
    if visualize:
        ...
        passport = get_passport_image(transparent_original, width_resize, aspect_ratio)
...
def save_images(transparent_image, image_name, folders_paths, _width_passport, _aspect_ratio):
    ...
    for key, path in folders_paths.items():
        ...
        if key == "transparent":
            ...
            passport_image = get_passport_image(transparent_image, _width_passport, _aspect_ratio)
            save_image(passport_image, save_path)            
            

def create_folders(_strSavePath, _bTransparent, _bWhiteBack, _bPassport):
    if not _strSavePath:
        return None
    
    save_path = _strSavePath      
...
def process_image(_img_path, _transparent, _white_back, _passport, _visualize, _always_matting = True, _width_resize = 600, _aspect_ratio = 1.285):
    transparent = remove_background(_img_path, _always_matting, _width_resize, _aspect_ratio, _visualize)
    strSavePath = getPath("processed")
    folders_paths = create_folders(strSavePath, _transparent, _white_back, _passport)
    image_name = os.path.split(_img_path)[1]
    save_images(transparent, image_name, folders_paths, _width_resize, _aspect_ratio)
