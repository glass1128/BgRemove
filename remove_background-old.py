...
model_matting = nn.DataParallel(model_matting).cuda().eval()
model_matting.load_state_dict(torch.load(pretrained_weight))
...
def remove_background(path, args, visualize = False):
    ...
    if visualize:
        ...
        passport = get_passport_image(transparent_original)
...
def save_images(transparent_image, image_name, folders_paths, args):
    ...
    for key, path in folders_paths.items():
        ...
        if key == "transparent":
            ...
            passport_image = get_passport_image(transparent_image, args.width_passport)
...
def main(args):        
    if args.folder:
        ...
        remove_background(args.image, args, args.visualize)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--width_passport", type=int, default=320)
